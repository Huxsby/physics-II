...
def main():
    ...
    parser.add_option('-a', '--ang', help=' ́Angulos de Euler finales para el elemento terminal en grados', action='store')
    ...
    parser.set_defaults(seed_joints='0 0 0 0 0 0', xyz='0.1 0.1 0.1', ang='0 0 0', eomg='0.01', ev=0.001)
    options, arguments = parser.parse_args()
#**************************************************************************************
    seed_joints=str(options.seed_joints).split()
    t=[]
    for i in range (0,6,1): t.append(np.float64(seed_joints[i]))

    xyz=str(options.xyz).split()
    r=[]
    for i in range (0,3,1): r.append(np.float64(xyz[i]))

    ang=str(options.ang).split()
    orientation=[]
    for i in range (0,3,1): orientation.append(np.deg2rad(np.float64(ang[i])))
    ...
    # Calculamos la Matriz de Transformación Homog ́enea a partir de posiciones y  ́angulos
    T=getT(orientation, r)
    #print(np.round(T,2))
    # Calculamos los ejes helicoidales en la posición cero del robot
    S=getS()
    #print(S)
    # Matriz de transformación homog ́enea en la posición cero del robot
